...
public class MovementComponent extends Component {
	 ...
	 * v(delta) = v / F^(delta) + a * (((1 / F^(delta)) - 1) / ((1 / F) - 1)),
	 * where F != 1.
	 * v(delta) = v / F^(delta) + a * delta, where F == 1.
	 */
	private float newVelocity(float velocity, float acceleration, float friction, float delta) {
	...
	/**
	 * To get the terminal velocity for given acceleration and friction the
	 * formula for newVelocity() is used.
	 * Calculate v(infinity) to get the terminal velocity.
	 * 
	 * The function can be written:
	 * v(infinity) ~ -a / ((1 / F) - 1)
	 */
	private float terminalVelocity(float acceleration, float friction) {
	 ...
	 * 
	 * This function can be rewritten to get the acceleration needed to obtain a
	 * given terminal velocity.
	 * a = -v(infinity) / F + v(infinity),
	 * where v(infinity) is the terminal velocity.
	 */
	private float calculateAcceleration(float friction, float terminalVelocity) {
	...
	public void update(float delta) {
		...
		/*
		 * double diagonal = Math.sqrt((direction.x * direction.x) +
		...
		position.y = position.y + delta / 2 * (oldVelocity.y + velocity.y);
		position.x = position.x + delta / 2 * (oldVelocity.x + velocity.x);
